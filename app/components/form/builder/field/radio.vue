...
const field = useFieldRef<RadioField>(props.field.id);
...
function isDuplicate(value: RadioField['options'][number]) {
  return field.value.options.some(
    (option) => option.id !== value.id && option.value === value.value.trim()
  );
}
</script>
  ...
  <div>
    ...
    <div>
      <Draggable
        ...
        @end="drag = false"
      >
        <template #item="{ element }: { element: RadioField['options'][number] }">
          <div>
            <div class="flex items-center gap-x-1">
              <div class="handle flex-center gap-x-2 cursor-move p-2">
                <NuxtIcon name="lucide:grip-vertical" />

                <div class="border-[1.6px] border-default rounded-full p-[0.46rem]" />
              </div>
