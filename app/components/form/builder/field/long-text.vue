...
const field = useFieldRef<LongTextField>(props.field.id);
...
const validationTypes = ref<LongTextValidation['type'][]>(['min_length', 'max_length']);

function addValidation() {
  const validation: LongTextValidation = {
    id: ID(),
    type: 'min_length',
    value: 10,
    message: ''
  ...
  <div>
    ...
    <div v-if="field.validations?.length">
      <Draggable
        ...
        @end="drag = false"
      >
        <template #item="{ element }: { element: LongTextValidation }">
          <div class="flex border border-default rounded-md pr-0.5">
            ...
            <div class="grow flex items-center gap-1">
              ...
              <NuxtInput v-model="element.value" variant="outline" placeholder="Value" size="sm" />
